...
-- The υ-multiset functor, but now indexing types are sets.

P∞ : ∀ {ℓl} υ → Type ℓl → Type (ℓ-max ℓl (ℓ-suc υ))
P∞ υ X = Σ[ A ∈ Type υ ] isSet A × (A → X)

setP∞ : ∀ {ℓ₁ ℓ₂} {X : Type ℓ₁} (X' : P∞ ℓ₂ X) → isSet ⟨ X' ⟩
setP∞ = fst ∘ snd
...
mapP∞ : ∀{ℓl₁ ℓl₂ υ} {X : Type ℓl₁} {Y : Type ℓl₂}
  → (X → Y)
  → P∞ υ X → P∞ υ Y
mapP∞ f (A , setA , ιA) = A , setA , f ∘ ιA

-- P∞ is a monad

ηP∞ : ∀{ℓl υ} {X : Type ℓl} → X → P∞ υ X
ηP∞ x = Unit* , isProp→isSet (λ _ _ → refl) , (λ _ → x)
  
bind : ∀{ℓl₁ ℓl₂ υ} {X : Type ℓl₁} {Y : Type ℓl₂}
  → (X → P∞ υ Y)
  → P∞ υ X → P∞ υ Y
bind f (A , setA , ιA) = (Σ[ a ∈ A ] f (ιA a) .fst) , isSetΣ setA (λ a → f (ιA a) .snd .fst) , uncurry (λ a → f (ιA a) .snd .snd)
...
bind∞ f Y = (Σ[ n ∈ ℕ ] bind* f n Y .fst) , isSetΣ isSetℕ (λ n → bind* f n Y .snd .fst) , uncurry (λ n → bind* f n Y .snd .snd)
